...
## Barcode-first Serial (pc + donate) — v0.9.6/0.9.7
Dell barcode encode serial มาเป๊ะ → **อ่าน barcode ก่อน OCR** (แก้ OCR สับสน `O↔0`/`5↔6` — Dell tag ไม่เคยมี `O`/`I`);
อ่านไม่ออก → fallback OCR. remark ผ่าน `serial_source` ("barcode"/"ocr") → Review **คอลัมน์ Src** + persist ลง `ground_truth.csv`.
- `read_serial_barcode(img, category)` — decoder **lazy: ZBar → Data Matrix → cv2** (เจอ valid แล้ว return ทันที):
  - `_decode_zbar` (`pyzbar`/ZBar) — **Code128** (desktop + laptop service tag), เร็ว ~0.03s; จำกัด `symbols=Code128/Code39/QR` (ตัด PDF417 assert-spam)
  - `_decode_datamatrix` (`pylibdmtx`) — **Data Matrix** (laptop/monitor; มี **ECC** = decode สำเร็จ = ถูกแน่); gray + upscale 1.5x + timeout 2s
  - `_decode_cv2_codes` (cv2 `QRCodeDetector` + `barcode`) — fallback (อ่าน Data Matrix ไม่ได้)
- `_pick_serial` — กรอง `CBA…` (bank asset tag, ไม่ใช่ serial เครื่อง) + validate ด้วย `_valid_dell_tag` (7-char alnum, ปัด IMEI/Express-code เลขล้วน)
- **scope = `pc` + `donate` + `monitor`** (monitor v1.0.0): จอ = **CN serial** จาก Data Matrix/Code128 → `_reformat_cn` ใส่ dash (`CN-…-A00`); 1D จอเอียง → `_read_barcode_rotated` (upscale 2x + ±3..±12°, วัด ~60% เช่น 20.jpg +6°); เลือกเฉพาะ payload รูป CN (ทิ้ง Service Tag 7-char). accessory **ตัด** (S/N พิมพ์/สลัก ไม่อยู่ในโค้ด → 0/40)
- ⚠️ ZBar-first ปลอดภัยกับ laptop Data Matrix: ZBar คืน CBA/QR-string ที่ไม่ผ่าน `_valid_dell_tag` → fall through ไป Data Matrix (พิสูจน์บน Train2)
- ⚠️ **single-source**: `read_serial_barcode` อยู่ใน `bulk_extract.py`, `ocr_worker.py` import ใช้ → wiring barcode-first ต้อง mirror 2 ไฟล์
- ผล: PC serial coverage **+10pp**; donate disagreement **8/8 barcode ถูก OCR ผิด** ([[Accuracy-Results]] · [[logs/2026-06-26-barcode-first-serial]])
- ⚠️ Code39/128 **ไม่มี ECC** แรงเท่า Data Matrix → ต้อง measurement-gate + spot-check ก่อนเชื่อ; dep ใหม่ offline ([[Build-and-Distribution]])
